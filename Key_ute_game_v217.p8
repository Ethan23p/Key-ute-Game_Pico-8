...
function create_level(level_title, seqOrder, coords_spawn, 
    ...
    levels[level_title] =
    {
        ...
        levelTimer = levelTimer or {timerMax = 300}
    }
end
...
function advance_level(level)
    ...
    if levelNext_seqOrder > #levels then
        troubleshooting("levelLoop","No more levels, looping")
        level_current = levels[level_initial]
    else
        for level, params in pairs(levels) do
            if params.seqOrder == levelNext_seqOrder then
                local wasFound_bool = true

                level_current = params
            end
        end
